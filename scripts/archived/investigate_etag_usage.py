...
def investigate_etags() -> None:
    ...
    urls = sorted(
        {feed_config["url"] for channel_config in instance["feeds"].values() for feed_config in channel_config.values()}
    )
    for url in urls[:10]:
        time.sleep(0.5)
        ...
        response1 = requests.get(
            ...
            headers={"User-Agent": config.USER_AGENT, "If-None-Match": "jhok23*%^&%^&D*$*7632984"},
        )
        ...
        if etag1:
            log.info(
                "Response 1 for %s has status %s, content length %s, and etag %s.",
                url,
            ...
            response2 = requests.get(
                url, timeout=config.REQUEST_TIMEOUT, headers={"User-Agent": config.USER_AGENT, "If-None-Match": etag1}
            )
            response2.raise_for_status()
            assert response2.status_code in (200, 304)
            etag2 = response2.headers.get("ETag")
            log.info(
                "Response 2 for %s has status %s, content length %s, and etag %s.",
                url,
